...
const create = async (request, response) => {
    ...
    try {
        ...
        const conclusion = await ConclusionFamilleTowLevTow.findOne({ observateurId: request.body.observateurId });

        if(conclusion) {

            await ConclusionFamilleTowLevTow.updateOne({ observateurId: request.body.observateurId }, {
                $set: {
                    a : request.body.a,  
                    b : request.body.b,  
                    c : request.body.c,  
...
const reset = async (request, response) => {
    ...
    try {
        ...
        const conclusion = await ConclusionFamilleTowLevTow.deleteOne({ observateurId : observateurId });
